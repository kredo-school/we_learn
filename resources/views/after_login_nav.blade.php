<nav class="flex items-center justify-between flex-wrap bg-white">
    ...
    <div class="flex items-center">
      {{-- Home button--}}
      <a href="{{ route('exchange.home') }}" class="text-sm text-black hover:text-[#ED4C59] px-4 py-2 mr-2 md:mr-6">
      ...
      <div class="relative relative-hover">
        <div class="flex items-center">
            <button class="text-sm text-black hover:text-[#ED4C59] px-4 py-2">
              <img src="{{ ($exchange->profile_img != null) ? $exchange->profile_img : '/assets/images/user-circle.png' }}" alt="User-pic" class="w-7 h-7 object-cover rounded-full" />
            </button>
            <span class="text-sm text-black mr-6 hover:text-[#ED4C59] px-4 py-2">Hello ! {{ $exchange->name }}</span>
          </div>

        <div class="absolute z-10 hidden bg-white rounded-lg shadow-md">
          <a href="{{ route('exchange.show.profile', ['id' => $exchange->id]) }}" class="block px-4 py-2 text-gray-800 hover:bg-gray-200 flex items-center">
            <img src="{{ ($exchange->profile_img != null) ? $exchange->profile_img : '/assets/images/user-circle.png' }}" alt="User-pic"
              class="w-4 h-4 object-cover rounded-full mr-2" />
            <span class="text-sm text-black">Profile</span>
          </a>
          <form method="POST" action="{{ route('exchange.logout') }}">
            @csrf
